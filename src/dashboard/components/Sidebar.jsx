...
const NAV_ITEMS = [
  ...
  { id: 'doc-analysis',label: 'Doc Analysis',        icon: FileSearch      },
  ...
  const NavItem = ({ item, isActive }) => {
    ...
    return (
      ...
      >
        <div className={`flex-shrink-0 relative ${collapsed ? 'mx-auto' : ''}`}>
          <Icon
            ...
            className={`transition-all ${isActive
              ? 'text-cyan-glow drop-shadow-[0_0_6px_rgba(0,245,255,0.8)]'
        ...
        {/* NEW badge for AI Engine and Doc Analysis */}
        {!collapsed && !isActive && (item.id === 'ai-engine' || item.id === 'doc-analysis') && (
          <span className="ml-auto font-mono text-[8px] bg-cyan-glow/15 text-cyan-glow border border-cyan-glow/25 px-1.5 py-0.5 rounded-md tracking-wider">
            NEW
          </span>
        )}
      </motion.button>
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-3 border-b border-white/5">
        <div className={`flex items-center gap-2.5 px-2 py-2 rounded-xl bg-green-500/5 border border-green-500/10
          ...
          <AnimatePresence>
            {!collapsed && (
              ...
              >
                ...
                <p className="font-mono text-[9px] text-slate-600 leading-tight truncate max-w-[130px]">
