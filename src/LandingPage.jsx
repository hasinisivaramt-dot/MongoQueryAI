...
import {
  ...
  Code2, TrendingUp, Target, Users, Timer,
  Check, BookOpen, Mail, ExternalLink, Link2,
} from 'lucide-react';

// ── Demo Modal ────────────────────────────────────────────────────────────────
function DemoModal({ onClose, onSignup }) {
  useEffect(() => {
  ...
  const steps = [
    {
      label: 'You type',
      content: '"Show all employees with salary above 50,000 in Engineering"',
      type: 'input',
    },
    {
      label: 'Claude AI generates',
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-full max-w-lg glass rounded-3xl border border-cyan-glow/20 overflow-hidden shadow-2xl shadow-black/60"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <span className="font-mono text-[10px] text-cyan-glow tracking-widest">LIVE DEMO</span>
            </div>
            <h3 className="font-display font-bold text-lg text-white">MongoQuery AI in Action</h3>
          </div>
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-xl glass border border-white/8 flex items-center justify-center text-slate-500 hover:text-white hover:border-white/20 transition-all"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-4">
          {/* Input */}
          <div>
            <p className="font-mono text-[10px] text-slate-600 uppercase tracking-widest mb-2">You type in plain English</p>
            <div className="glass border border-cyan-glow/15 rounded-2xl px-4 py-3.5">
              <p className="font-body text-sm text-slate-200 leading-relaxed">
                "Show all employees with salary above 50,000 in Engineering"
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 py-1">
            <div className="h-px flex-1 bg-gradient-to-r from-transparent to-cyan-glow/40" />
            <motion.div
              animate={{ scale: [1, 1.05, 1] }}
              transition={{ duration: 1.5, repeat: Infinity }}
              className="badge-info px-3 py-1.5 rounded-full font-mono text-[10px] flex items-center gap-2 flex-shrink-0"
            >
              <Brain size={10} className="text-cyan-glow" />
              Claude AI · 340ms
            </motion.div>
            <div className="h-px flex-1 bg-gradient-to-l from-transparent to-cyan-glow/40" />
          </div>

          {/* Generated query */}
          <div>
            <p className="font-mono text-[10px] text-slate-600 uppercase tracking-widest mb-2">Generated MongoDB query</p>
            <div className="bg-black/50 border border-cyan-glow/15 rounded-2xl overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/5">
                <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                <span className="font-mono text-[10px] text-green-400">Query ready · 99.2% confidence</span>
              </div>
              <pre className="px-4 py-4 font-mono text-sm text-green-300 overflow-x-auto">{`db.employees.find({
  dept: "Engineering",
  salary: { $gt: 50000 }
}).sort({ salary: -1 })`}</pre>
            </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { val: '145',   sub: 'docs returned'  },
              { val: '38ms',  sub: 'execution time' },
              { val: '99.2%', sub: 'AI accuracy'    },
            ].map(s => (
              <div key={s.sub} className="glass border border-white/8 rounded-xl p-3 text-center">
                <p className="font-display font-black text-lg text-cyan-glow">{s.val}</p>
                <p className="font-mono text-[9px] text-slate-500 mt-0.5">{s.sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {['Table View', 'JSON Export', 'CSV Export', 'Charts'].map(f => (
              <span key={f} className="font-mono text-[10px] px-2.5 py-1 rounded-lg bg-white/4 border border-white/8 text-slate-400">{f}</span>
            ))}
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-white/5 flex flex-col sm:flex-row gap-3">
          <motion.button
            ...
            className="btn-primary flex-1 py-3 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2"
          >
            <Zap size={15} />
          ...
          <button
            onClick={onClose}
            className="btn-outline flex-1 py-3 rounded-2xl font-medium text-sm"
          >
            Maybe Later
          </button>
        </div>
...
function QueryCard() {
  ...
  const colorMap = {
    comment: 'text-slate-500', kw: 'text-purple-400', str: 'text-green-400',
    brk: 'text-yellow-400', ind: 'text-slate-300', norm: 'text-slate-300',
  };
  ...
  return (
    <motion.div
      ...
      style={{ perspective: '1000px', maxWidth: '520px' }}
    >
      <motion.div
        animate={{ y: [0, -12, 0] }}
      ...
      >
        ...
        {/* Code lines */}
        <div className="p-4 sm:p-5 code-block space-y-0.5 overflow-x-auto">
          {lines.map((line, i) => (
            <motion.div
              key={i}
              ...
              className={`${colorMap[line.t]} text-xs leading-6 whitespace-nowrap`}
            >
        ...
        <motion.div
          ...
          className="px-4 sm:px-5 py-3 bg-green-500/5 border-t border-green-500/10 flex items-center gap-3"
        >
          <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
      ...
      <motion.div
        ...
        className="absolute -top-4 -right-4 glass border-glow rounded-xl px-3 py-2 flex items-center gap-2 hidden sm:flex"
      >
      ...
      <motion.div
        ...
        className="absolute -bottom-4 -left-4 glass border-glow rounded-xl px-3 py-2 flex items-center gap-2 hidden sm:flex"
      >
        <div className="w-2 h-2 rounded-full bg-cyan-glow animate-pulse" />
...
// ── Hero ──────────────────────────────────────────────────────────────────────
function Hero({ onSignup, onLogin, onDemo }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] sm:w-[800px] h-[600px] sm:h-[800px] rounded-full border border-cyan-glow/5 animate-pulse" />

      {/* ── Two-column grid — stacks on mobile ── */}
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16 lg:py-20 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">

        {/* Left column */}
        <div className="text-left">
          <motion.div
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
            className="inline-flex items-center gap-2 glass border-glow rounded-full px-4 py-2 mb-6 sm:mb-8"
          >
            <div className="w-2 h-2 rounded-full bg-cyan-glow animate-pulse" />
          ...
          <motion.h1
            ...
            className="font-display font-black text-4xl sm:text-5xl lg:text-7xl leading-[1.05] tracking-tight mb-5 sm:mb-6"
          >
            ...
            <span className="text-slate-200">
              {typed}<span className="text-cyan-glow">|</span>
            </span>
          </motion.h1>
          ...
          <motion.p
            ...
            className="font-body text-base sm:text-lg text-slate-400 leading-relaxed mb-8 sm:mb-10 max-w-xl"
          >
          ...
          <motion.div
            ...
            className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-3 sm:gap-4 mb-10 sm:mb-12"
          >
            <motion.button
              whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
              onClick={onSignup}
              className="btn-primary w-full sm:w-auto px-6 sm:px-8 py-4 rounded-2xl text-base font-semibold flex items-center justify-center gap-3 shadow-2xl shadow-blue-electric/30"
            >
              <Sparkles size={18} />Try MongoQuery Free<ArrowRight size={16} />
            </motion.button>

            {/* Watch Demo — opens modal, NOT signup */}
            <motion.button
              whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
              onClick={onDemo}
              className="btn-outline w-full sm:w-auto px-6 sm:px-8 py-4 rounded-2xl text-base font-medium flex items-center justify-center gap-3"
            >
              <div className="w-8 h-8 rounded-full bg-cyan-glow/10 border border-cyan-glow/30 flex items-center justify-center flex-shrink-0">
...
// ── Feature Card (extracted to fix useRef-in-map React violation) ─────────────
const FEATURES = [
...
// Each card is its own component so useRef/useInView are called at component level (not inside map)
function FeatureCard({ f, i }) {
  ...
  return (
    <motion.div
      ...
      className="group relative glass border-glow-hover rounded-2xl p-5 sm:p-6 cursor-pointer overflow-hidden"
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <h3 className="font-display font-bold text-base sm:text-lg text-white mb-2 sm:mb-3 group-hover:text-cyan-glow/90 transition-colors duration-300">{f.title}</h3>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        <div ref={ref} className="text-center mb-12 sm:mb-20">
          ...
          <motion.h2
            ...
            className="font-display font-black text-3xl sm:text-5xl lg:text-6xl text-white mb-4 sm:mb-6 tracking-tight"
          >
...
function StatCard({ s, i }) {
  ...
  return (
    <motion.div
      ...
      className="group relative glass border-glow-hover rounded-3xl p-6 sm:p-8 text-center overflow-hidden"
    >
      ...
      <div className="relative inline-flex mb-4 sm:mb-6">
        <div
          className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br ${s.gradient} flex items-center justify-center shadow-lg`}
      ...
      <div
        className="font-display font-black text-4xl sm:text-5xl lg:text-6xl mb-2 sm:mb-3 tracking-tight"
        style={{ color: s.color, textShadow: `0 0 30px ${s.color}60` }}
      >
        <Counter target={s.value} suffix={s.suffix} isInView={inView} />
      </div>
      <div className="font-display font-bold text-lg sm:text-xl text-white mb-1">{s.label}</div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        <div ref={ref} className="text-center mb-12 sm:mb-16">
          ...
          <motion.h2
            initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}}
            transition={{ delay: 0.1 }}
            className="font-display font-black text-3xl sm:text-5xl lg:text-6xl text-white tracking-tight"
          >
        ...
        >
          <p className="font-body text-sm text-slate-600 mb-4 sm:mb-6 tracking-wide">TRUSTED BY TEAMS AT</p>
          <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-8 lg:gap-14">
            {['Stripe', 'Shopify', 'Notion', 'Vercel', 'Linear', 'Figma'].map(c => (
              <span key={c} className="font-display font-bold text-lg sm:text-xl text-slate-700 hover:text-slate-400 transition-colors cursor-pointer tracking-tight">{c}</span>
            ))}
          </div>
        </motion.div>
      </div>
...
const PLANS = [
  {
    name: 'Starter', price: { m: 0, a: 0 }, tagline: 'Perfect for side projects',
    color: '#4d9eff', border: 'rgba(77,158,255,0.2)', popular: false,
    features: ['500 queries / month', 'Natural language to MongoDB', 'Basic aggregation pipelines', '1 database connection', 'Community support'],
    cta: 'Start for Free',
  },
  {
    name: 'Pro', price: { m: 49, a: 39 }, tagline: 'For professional developers',
    color: '#00f5ff', border: 'rgba(0,245,255,0.4)', popular: true, badge: 'Most Popular',
    features: ['Unlimited queries', 'Advanced AI query generation', 'Vector search integration', 'Real-time analytics dashboard', 'Up to 10 database connections', 'Pipeline visual builder', 'Smart visualizations', 'Slack + Email support'],
    cta: 'Start Pro Trial',
  },
  {
    name: 'Enterprise', price: { m: 249, a: 199 }, tagline: 'For teams at scale',
    color: '#a855f7', border: 'rgba(168,85,247,0.3)', popular: false,
    features: ['Everything in Pro', 'Unlimited connections', 'SOC 2 Type II compliance', 'VPC peering & private endpoints', 'Field-level encryption', 'Custom AI model fine-tuning', 'RBAC & SSO/SAML', 'Priority 24/7 support'],
    cta: 'Contact Sales',
  },
];
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        <div ref={ref} className="text-center mb-12 sm:mb-16">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-3 sm:gap-4 glass border-glow rounded-full px-4 sm:px-6 py-3"
          >
            <span
        ...
        {/* Plans — 1 col mobile, 2 col tablet, 3 col desktop */}
            ...
            return (
              <motion.div
                ...
                className={`relative flex flex-col rounded-3xl overflow-hidden ${
                  plan.popular
                    ? 'popular-card lg:scale-105 z-10 shadow-2xl'
                    : 'glass'
                }`}
                style={{
              ...
              >
                ...
                <div className={`p-6 sm:p-8 flex flex-col h-full ${plan.popular ? 'pt-10 sm:pt-12' : ''}`}>
                  ...
                  <div className="mb-6 sm:mb-8 pb-6 sm:pb-8 border-b" style={{ borderColor: `${plan.color}15` }}>
                  ...
                  <motion.button
                    whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                    onClick={onSignup}
                    className={`w-full py-3.5 sm:py-4 rounded-2xl font-semibold font-body text-sm flex items-center justify-center gap-2 transition-all ${
                      plan.popular ? 'btn-primary shadow-xl' : 'btn-outline'
                    }`}
        ...
        <motion.p
          ...
          className="text-center mt-8 sm:mt-10 font-body text-sm text-slate-600"
        >
  ...
  return (
    <section className="relative py-20 sm:py-32 overflow-hidden">
      ...
      <motion.div
        ...
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] sm:w-[800px] h-[300px] sm:h-[400px] orb bg-blue-electric"
      />
      ...
      <div ref={ref} className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center">
        <motion.h2
          ...
          transition={{ delay: 0.1 }}
          className="font-display font-black text-3xl sm:text-5xl lg:text-7xl text-white mb-4 sm:mb-6 tracking-tight leading-[1.05]"
        >
        ...
        <motion.p
          initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: 0.2 }}
          className="font-body text-base sm:text-xl text-slate-400 mb-8 sm:mb-12 max-w-2xl mx-auto"
        >
        ...
        <motion.div
          initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: 0.3 }}
          className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-4 sm:gap-5 mb-8 sm:mb-12"
        >
          <motion.button
            whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
            onClick={onSignup}
            className="btn-primary w-full sm:w-auto px-8 sm:px-10 py-4 sm:py-5 rounded-2xl text-base sm:text-lg font-semibold flex items-center justify-center gap-3 shadow-2xl shadow-blue-electric/40"
          >
            <Sparkles size={20} />Get Started Free<ArrowRight size={18} />
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
            className="btn-outline w-full sm:w-auto px-8 sm:px-10 py-4 sm:py-5 rounded-2xl text-base sm:text-lg font-medium flex items-center justify-center gap-3"
          >
        ...
        <motion.div
          ...
          className="flex flex-wrap items-center justify-center gap-4 sm:gap-8 text-slate-600"
        >
...
export default function LandingPage({ onLogin, onSignup, onNavigate }) {
  ...
  return (
    <div className="relative min-h-screen bg-navy-950 text-white overflow-x-hidden">
      ...
      <Navbar onLogin={onLogin} onSignup={onSignup} onNavigate={onNavigate} activePage="landing" />
      <Hero onSignup={onSignup} onLogin={onLogin} onDemo={() => setShowDemo(true)} />
      <Features />
      <StatsSection />
